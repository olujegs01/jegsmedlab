...
Generates a professional JegsMedLab report PDF from lab data.
"""
...
def generate_report_pdf(
    ...
    """
    Generate a PDF from a JegsMedLab report.
    ...
    story.append(Paragraph("JegsMedLab", header_style))
    ...
    disclaimer = (
        "⚠ This report is generated by JegsMedLab for educational purposes only. "
...
def generate_letter_pdf(letter_text: str, patient: dict, report: dict) -> bytes:
    """Generate a PDF for a referral letter."""
    buf = BytesIO()
    doc = SimpleDocTemplate(
        buf,
        pagesize=letter,
    ...
    # Header
    header_style = ParagraphStyle("LHdr", parent=styles["Normal"],
        fontSize=18, textColor=BRAND_BLUE, fontName="Helvetica-Bold", spaceAfter=2)
    sub_style = ParagraphStyle("LSub", parent=styles["Normal"],
        fontSize=9, textColor=colors.HexColor("#64748b"), spaceAfter=16)
    body_style = ParagraphStyle("LBody", parent=styles["Normal"],
        fontSize=10, leading=16, textColor=BRAND_DARK, spaceAfter=8)

    story.append(Paragraph("JegsMedLab", header_style))
    story.append(Paragraph("AI-Powered Lab Result Platform | jegsmedlab.vercel.app", sub_style))
    story.append(HRFlowable(width="100%", thickness=1, color=BRAND_BLUE, spaceAfter=16))

    # Letter body
    cleaned = clean_markdown(letter_text)
    ...
    disclaimer_style = ParagraphStyle("LDisc", parent=styles["Normal"],
        fontSize=7.5, textColor=colors.HexColor("#94a3b8"), leading=11)
    story.append(Paragraph(
        "This letter was generated by JegsMedLab AI for informational purposes only. "
        "It does not constitute a medical diagnosis or treatment recommendation. "
        "Please review with a licensed healthcare provider before taking any action.",
        disclaimer_style,
    ))
