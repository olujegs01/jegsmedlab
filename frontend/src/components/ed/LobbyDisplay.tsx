...
import { ED_WS, ED_API } from "@/contexts/EDAuthContext";
...
function ESIBar({ esi, count, waitMin }: any) {
  const cfg = ESI_CONFIG[esi];
  if (count === 0) return null;
  return (
    <div style={{ background: cfg.bg, border: `1px solid ${cfg.color}22`, borderLeft: `4px solid ${cfg.color}`, borderRadius: 10, padding: "18px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      ...
      <div style={{ fontSize: 13, color: "#334155" }}>{count === 1 ? "1 patient" : `${count} patients`}</div>
    </div>
  );
}

function Ticker({ events }: any) {
  if (!events.length) return null;
  return (
    <div style={{ background: "#0d1526", borderTop: "1px solid #1e293b", padding: "10px 32px", overflow: "hidden", whiteSpace: "nowrap" }}>
      <span style={{ color: "#334155", fontSize: 13, marginRight: 24, fontWeight: 600 }}>LIVE</span>
      <span style={{ color: "#475569", fontSize: 13 }}>{events.slice(-6).join("  ·  ")}</span>
    </div>
  );
}
...
export default function LobbyDisplay() {
  const [stats, setStats] = useState({
    queue: { total_patients: 0, esi_breakdown: {} as any, avg_wait_minutes: 0 },
  ...
  const fetchStats = useCallback(async () => {
    try {
      await fetch(`${ED_API}/analytics`, { headers: { Authorization: "Bearer lobby-public" } }).catch(() => null);
    } catch (_) {}
  }, []);

  const handleWsMsg = useCallback((msg: any) => {
    if (msg.event === "patient_added") {
      setTicker(prev => [...prev, `Patient checked in — Room ${msg.room_assignment || "assigned"} · ESI ${msg.esi_level}`]);
      setStats(prev => {
        const esi = msg.esi_level;
        const breakdown = { ...prev.queue.esi_breakdown };
        breakdown[String(esi)] = (breakdown[String(esi)] || 0) + 1;
        return { ...prev, queue: { ...prev.queue, total_patients: prev.queue.total_patients + 1, esi_breakdown: breakdown } };
      });
    } else if (msg.event === "patient_discharged") {
      setTicker(prev => [...prev, "Patient discharged — bed now available"]);
      setStats(prev => ({ ...prev, queue: { ...prev.queue, total_patients: Math.max(0, prev.queue.total_patients - 1) } }));
  ...
  const { connected } = useWebSocket(`${ED_WS}/ws/lobby`, handleWsMsg);
  ...
  return (
    <div style={{ background: "#060b14", minHeight: "100vh", display: "flex", flexDirection: "column", fontFamily: "'Inter', -apple-system, sans-serif" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "24px 40px", borderBottom: "1px solid #0d1526" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 8, height: 8, borderRadius: "50%", background: connected ? "#22c55e" : "#f59e0b", boxShadow: connected ? "0 0 6px #22c55e" : "none" }} />
      ...
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 320px", gap: 0 }}>
        <div style={{ padding: "32px 40px", borderRight: "1px solid #0d1526" }}>
          <div style={{ fontSize: 13, color: "#334155", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 20 }}>Current Queue</div>
          {!hasPatients ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {[1, 2, 3, 4, 5].map(esi => (
                <ESIBar key={esi} esi={esi} count={q.esi_breakdown?.[String(esi)] || 0} waitMin={esi <= 2 ? 0 : esi === 3 ? q.avg_wait_minutes : null} />
              ))}
            </div>
          )}
        </div>
        ...
        <div style={{ padding: "32px 28px", display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          <div style={{ marginTop: "auto", fontSize: 11, color: "#1e293b", lineHeight: 1.6 }}>
            For medical emergencies, alert staff immediately. If your condition worsens while waiting, please notify the triage desk at once.
          </div>
        </div>
      </div>
