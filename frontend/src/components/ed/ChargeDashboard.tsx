...
const ESI_COLORS: any = { 1: "#dc2626", 2: "#ea580c", 3: "#ca8a04", 4: "#16a34a", 5: "#6b7280" };
const ESI_LABELS: any = { 1: "Critical", 2: "High", 3: "Urgent", 4: "Less Urgent", 5: "Non-Urgent" };

function BigStat({ label, value, color = "#0d9488", sub, pulse }: any) {
  return (
    <div style={{ background: "#060e1a", borderRadius: 16, padding: "28px 24px", flex: 1, border: `1px solid ${color}30`, position: "relative", overflow: "hidden" }}>
      {pulse && <div style={{ position: "absolute", top: 12, right: 12, width: 10, height: 10, borderRadius: "50%", background: color, animation: "ping 1s infinite" }} />}
...
function WaitBar({ name, esi, waitMin, room, escalated }: any) {
  const color = ESI_COLORS[esi] || "#6b7280";
  const maxWait = esi <= 2 ? 30 : esi === 3 ? 120 : 240;
  const pct = Math.min(100, (waitMin / maxWait) * 100);
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "10px 0", borderBottom: "1px solid #1e293b", background: escalated ? "rgba(220,38,38,0.05)" : "transparent" }}>
      <div style={{ width: 32, height: 32, borderRadius: "50%", background: color + "20", border: `2px solid ${color}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color, flexShrink: 0 }}>{esi}</div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
          ...
          <span style={{ fontSize: 13, color: waitMin > (esi <= 2 ? 5 : 60) ? "#dc2626" : "#94a3b8", flexShrink: 0, marginLeft: 8 }}>{waitMin} min</span>
        </div>
...
export default function ChargeDashboard({ user }: any) {
  const [data, setData] = useState<any>(null);

  const fetchData = useCallback(async () => {
    try {
      const r = await fetch(`${ED_API}/charge/dashboard`, { headers: { Authorization: `Bearer ${user?.token}` } });
  ...
  if (!data) {
    return <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "60vh", color: "#475569", fontSize: 18 }}>Loading charge dashboard…</div>;
  }
  ...
  return (
    <div style={{ padding: "24px 32px", minHeight: "100vh", background: "#030711" }}>
      ...
      {escalated_count > 0 && (
        <div style={{ background: "rgba(220,38,38,0.15)", border: "1px solid #dc2626", borderRadius: 12, padding: "16px 24px", marginBottom: 24, display: "flex", alignItems: "center", gap: 16 }}>
          <span style={{ fontSize: 28 }}>🚨</span>
          <div>
            <div style={{ fontSize: 16, fontWeight: 700, color: "#f87171" }}>{escalated_count} patient{escalated_count > 1 ? "s" : ""} with wait time violation</div>
            <div style={{ fontSize: 13, color: "#94a3b8" }}>ESI 1–2 patients waiting &gt;5 min · ESI 3 patients waiting &gt;60 min</div>
          </div>
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 24 }}>
        <div style={{ background: "#060e1a", borderRadius: 16, padding: 24, border: "1px solid #1e293b" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {[1, 2, 3, 4, 5].map(esi => {
              ...
              const hasEscalated = pts.some((p: any) => p.escalated);
              return (
                <div key={esi} style={{ display: "flex", alignItems: "center", gap: 16 }}>
                  <div style={{ width: 40, height: 40, borderRadius: 8, background: color + "20", border: `2px solid ${color}`, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, color, fontSize: 14, flexShrink: 0 }}>ESI{esi}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
        ...
        <div style={{ background: "#060e1a", borderRadius: 16, padding: 24, border: "1px solid #1e293b" }}>
          ...
          {longest_waits.length === 0 ? (
            ...
            longest_waits.map((p: any, i: number) => (
      ...
      <div style={{ background: "#060e1a", borderRadius: 16, padding: 24, border: "1px solid #1e293b" }}>
        ...
        <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1, display: "flex", alignItems: "center" }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 700, color: "#e2e8f0", marginLeft: 12, flexShrink: 0 }}>{bed_summary.occupancy_percent}%</span>
          </div>
        </div>
      </div>
