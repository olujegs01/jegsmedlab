...
import { ED_API, useEDAuth } from "@/contexts/EDAuthContext";
...
function JourneyRow({ journey, onCheckin, onResolve, checkinLoading, resolveLoading }: any) {
  const [expanded, setExpanded] = useState(false);
  const cfg = STATUS_CONFIG[journey.journey_status] || STATUS_CONFIG.active;

  const nextCheckin = journey.next_checkin_at ? new Date(journey.next_checkin_at) : null;
  const isPast = nextCheckin && nextCheckin < new Date();
  const dischargedAgo = journey.discharge_at
    ? Math.round((Date.now() - new Date(journey.discharge_at).getTime()) / 3600000)
    : null;

  return (
    <div className={`journey-row ${journey.journey_status}`}>
      <div className="journey-row-main" onClick={() => setExpanded((e: boolean) => !e)}>
        ...
        <div className="journey-checkin-progress">
          <div className="journey-checkin-dots">
            {Array.from({ length: journey.checkins_total }).map((_: any, i: number) => (
              <div key={i} className={`checkin-dot ${i < journey.checkins_completed ? "done" : ""}`} />
            ))}
          </div>
        ...
        <div className="journey-detail">
          ...
          {journey.last_response && (
            <div className="journey-last-response">
              ...
              {journey.last_checkin_at && (
                <span className="journey-response-time"> · {new Date(journey.last_checkin_at).toLocaleString()}</span>
              )}
            </div>
          )}
          {journey.checkin_log?.length > 0 && (
            <div className="journey-log">
              ...
              {journey.checkin_log.slice().reverse().map((entry: any, i: number) => (
          ...
          <div className="journey-actions">
            {journey.journey_status !== "completed" && (
              <button className="journey-btn primary" onClick={() => onCheckin(journey.journey_id)} disabled={checkinLoading}>
                {checkinLoading ? "Sending…" : "📤 Send Check-in Now"}
              </button>
            )}
            {journey.journey_status === "escalated" && (
              <button className="journey-btn warning" onClick={() => onResolve(journey.journey_id)} disabled={resolveLoading}>
...
function CompliancePage({ token }: any) {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${ED_API}/compliance/status`, { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()),
      fetch(`${ED_API}/compliance/escalation-rules`, { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()),
    ]).then(([s, r]) => {
      setStatus(s);
  ...
  const toggleRule = async (ruleId: any, enabled: boolean) => {
    setSaving(ruleId);
    const res = await fetch(`${ED_API}/compliance/escalation-rules/${ruleId}`, {
      method: "PATCH",
    ...
    if (res.ok) {
      ...
      setRules((r: any[]) => r.map((x: any) => x.id === ruleId ? updated : x));
    }
    setSaving(null);
  };

  const downloadBAA = () => { window.open(`${ED_API}/compliance/baa`, "_blank"); };
  ...
  return (
    <div className="compliance-page">
      <div className="compliance-hero">
        ...
        <div className="compliance-score-info">
          ...
          <div className="compliance-badges">
            ...
            <span className="comp-badge baa" onClick={downloadBAA} style={{ cursor: "pointer" }}>📄 Download BAA</span>
          </div>
        </div>
      </div>
      ...
      <div className="compliance-controls">
        {status?.hipaa_controls?.map((ctrl: any, i: number) => (
          ...
          <div key={rule.id} className={`esc-rule ${rule.enabled ? "enabled" : "disabled"}`}>
            <div className="esc-rule-main">
              ...
              <label className="toggle-switch">
                <input type="checkbox" checked={rule.enabled} disabled={saving === rule.id} onChange={e => toggleRule(rule.id, e.target.checked)} />
...
export default function ClinicalJourneys({ activeTab }: any) {
  ...
  const load = useCallback(() => {
    const url = filter === "all" ? `${ED_API}/journeys` : `${ED_API}/journeys?status=${filter}`;
    fetch(url, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json())
      .then((data: any) => { setJourneys(Array.isArray(data) ? data : []); setLoading(false); })
      .catch(() => setLoading(false));
  }, [token, filter]);

  useEffect(() => { load(); }, [load]);

  const [checkinLoading, setCheckinLoading] = useState<any>(null);
  const [resolveLoading, setResolveLoading] = useState<any>(null);
  ...
  const handleCheckin = async (journeyId: any) => {
    setCheckinLoading(journeyId);
    try {
      const res = await fetch(`${ED_API}/journeys/${journeyId}/checkin`, { method: "POST", headers: { Authorization: `Bearer ${token}` } });
      const data = await res.json();
      if (res.ok) { showToast(data.sms_sent ? "SMS check-in sent to patient." : "Check-in logged (SMS not configured).", true); }
      else { showToast(data.detail || "Check-in failed.", false); }
      load();
    } catch { showToast("Network error — please retry.", false); }
    finally { setCheckinLoading(null); }
  };

  const handleResolve = async (journeyId: any) => {
    setResolveLoading(journeyId);
    try {
      const res = await fetch(`${ED_API}/journeys/${journeyId}/resolve`, { method: "POST", headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) { showToast("Journey resolved and marked completed.", true); }
      else { showToast("Failed to resolve journey.", false); }
      load();
    } catch { showToast("Network error — please retry.", false); }
  ...
  const escalated = journeys.filter((j: any) => j.journey_status === "escalated");
  const active    = journeys.filter((j: any) => j.journey_status === "active");
  const completed = journeys.filter((j: any) => j.journey_status === "completed");
    ...
    <div className="journeys-page">
      ...
      <div className="journey-filter-tabs">
        {["all", "escalated", "active", "completed"].map(f => (
          <button key={f} className={`journey-filter-tab ${filter === f ? "active" : ""}`} onClick={() => setFilter(f)}>
      ...
      {loading ? (
        ...
        <div className="journey-list">
          {filtered.map((j: any) => (
            <JourneyRow key={j.journey_id} journey={j} onCheckin={handleCheckin} onResolve={handleResolve}
              checkinLoading={checkinLoading === j.journey_id} resolveLoading={resolveLoading === j.journey_id} />
          ))}
        </div>
      )}
    </div>
