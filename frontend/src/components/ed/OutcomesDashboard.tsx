...
const ESI_COLORS: any = { ESI1: "#dc2626", ESI2: "#ea580c", ESI3: "#ca8a04", ESI4: "#16a34a", ESI5: "#6b7280" };
const CARE_COLORS: any = {
...
function KPICard({ label, value, unit = "", sub, color = "#0d9488", benchmark }: any) {
  return (
...
function BenchmarkRow({ metric, yours, national, unit, lower_is_better }: any) {
...
function SepsisComponents({ components }: any) {
  return (
...
function exportCSV(data: any) {
  ...
  const rows: any[] = [["Report", "MediScan Gateway — Outcomes Export"], ["Generated", new Date().toISOString()], []];
  rows.push(["--- 7-Day Volume Trends ---"]);
  rows.push(["Date", "Patients", "Avg Wait (min)", "Avg LOS (min)", "LWBS Rate (%)"]);
  (trends_7d || []).forEach((r: any) => rows.push([r.date, r.patients, r.avg_wait_min, r.avg_los_min, r.lwbs_rate]));
  rows.push([]);
  rows.push(["--- 7-Day ESI Distribution ---"]);
  rows.push(["Date", "ESI1", "ESI2", "ESI3", "ESI4", "ESI5"]);
  (esi_trends || []).forEach((r: any) => rows.push([r.date, r.ESI1, r.ESI2, r.ESI3, r.ESI4, r.ESI5]));
  rows.push([]);
  rows.push(["--- 24h Hourly Volume ---"]);
  rows.push(["Time", "Patients", "Wait (min)"]);
  (timeseries || []).forEach((r: any) => rows.push([r.time, r.patients, r.wait_min]));
...
function SatisfactionSection({ user }: any) {
  const [sat, setSat] = useState<any>(null);
  useEffect(() => {
    fetch(`${ED_API}/outcomes/satisfaction`, {
  ...
  const stars = Array.from({ length: 5 }, (_: any, i: number) => i + 1);
  const maxDist = Math.max(...Object.values(sat.distribution) as number[]);
    ...
    <Section title="Patient Satisfaction" subtitle={`${sat.total_responses} responses · avg ${sat.avg_rating}/5`}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
        ...
        <div>
          ...
          {Object.entries(sat.by_category).map(([cat, avg]: any) => (
          ...
          {sat.recent_comments?.length > 0 && (
            <div style={{ marginTop: 16 }}>
              ...
              {sat.recent_comments.slice(0, 3).map((c: any, i: number) => (
...
export default function OutcomesDashboard({ user }: any) {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await fetch(`${ED_API}/analytics`, {
        headers: { Authorization: `Bearer ${user?.token}` },
      });
    ...
    <div className="outcomes-dashboard">
      ...
      <div className="dash-toolbar">
        ...
        <div className="dash-toolbar-right">
          ...
          <button className="dash-export-btn" onClick={() => window.open(`${ED_API}/report?format=pdf&token=${user?.token}`, "_blank")}>
            ↓ Export PDF
          </button>
        </div>
      </div>
        ...
        <Section title="Current ESI Mix" subtitle="Active queue breakdown">
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              ...
              >
                {[1,2,3,4,5].map(e => (
                  <Cell key={e} fill={Object.values(ESI_COLORS)[e - 1] as string} />
                ))}
              </Pie>
              <Tooltip content={<DarkTooltip />} />
              <Legend wrapperStyle={{ fontSize: 11, color: "#64748b" }} />
          ...
          <div className="diversion-content">
            <ResponsiveContainer width="50%" height={200}>
              <PieChart>
                ...
                >
                  {diversion.breakdown.map((b: any, i: number) => (
